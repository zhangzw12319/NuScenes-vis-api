...
class SemLaserScan(LaserScan):
  ...
  def __init__(self, nclasses, sem_color_dict=None, project=False, H=64, W=1024, fov_up=3.0, fov_down=-25.0, learning_map=None):
  ...
  def set_label(self, label):
    ...
    if label.shape[0] == self.points.shape[0]:
      ...
      self.sem_label = label // 1000
      # raw semantic label mapped to 0-16
      if self.learning_map is not None:
        self.sem_label = np.vectorize(self.learning_map.__getitem__)(self.sem_label)
      self.inst_label = label % 1000
    else:
    ...
    # sanity check
    # assert((self.sem_label + (self.inst_label << 16) == label).all()) # for semantickitti
